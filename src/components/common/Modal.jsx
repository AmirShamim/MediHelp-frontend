...
const Modal = ({ open, onClose, title, children, footer }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="modal-title" aria-live="assertive">
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-lg shadow-lg border animate-[fadeIn_.18s_ease] max-h-[90vh] flex flex-col" role="document">
        <div className="px-5 py-4 border-b flex items-center justify-between">
          <h3 id="modal-title" className="text-lg font-semibold text-gray-800">{title}</h3>
          <button onClick={onClose} aria-label="Close dialog" className="text-gray-500 hover:text-gray-700 p-1 rounded focus-ring">
